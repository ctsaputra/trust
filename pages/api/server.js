...
app.get('/gogo', (req, res) => {
  if (req.session.isAuthenticated) {
    ...
    res.sendFile(path.join(process.cwd(), 'gogo', 'index.html'));
  }
});
...
app.use('/redirect', express.static(path.join(process.cwd(), 'redirect')));

app.get('/redirect/*', requireAuth, (req, res) => {
  res.sendFile(path.join(process.cwd(), 'redirect', 'index.html'));
});
